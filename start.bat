...
start "Dir-Server"  cmd /k "cd /d "%~dp0" && uv run Servers\directory_server.py"
timeout /t 1 /nobreak >nul

start "Chat-Server" cmd /k "cd /d "%~dp0" && uv run Servers\chat_server.py --port 8001"
timeout /t 1 /nobreak >nul
...
for /L %%i in (1,1,%NODE_COUNT%) do (
    ...
    start "Entry-Node-%%i"  cmd /k "cd /d "%~dp0" && uv run Servers\node.py --type entry  --port !ENTRY_PORT!  %DEBUG_FLAG%"
    timeout /t 1 /nobreak >nul
    start "Middle-Node-%%i" cmd /k "cd /d "%~dp0" && uv run Servers\node.py --type middle --port !MIDDLE_PORT! %DEBUG_FLAG%"
    timeout /t 1 /nobreak >nul
    start "Exit-Node-%%i"   cmd /k "cd /d "%~dp0" && uv run Servers\node.py --type exit   --port !EXIT_PORT!   %DEBUG_FLAG%"
    timeout /t 1 /nobreak >nul
)

timeout /t 1 /nobreak >nul
echo start "Chat-Client-Tor" cmd /k "cd /d "%~dp0" && uv run Servers\chat_client.py --tor --port 8001"
